...
ContextChain::ContextChain(const SEALContext& base_context, size_t e, MemoryPoolHandle pool, sec_level_type sec_level)
{
	...
	for (size_t i = 1; i < e; ++i) {
		...
		contexts.emplace_back(new_parms, false, sec_level);
		evaluators.emplace_back(std::make_unique<Evaluator, const SEALContext&>(contexts.back()));
	}
...
void ContextChain::multiply_switch_inplace(Ciphertext& value, size_t power) const
{
	const size_t context_index = get_context_index(value.parms_id());
	const SEALContext& source_context = contexts[context_index];
	...
	if (context_index + power >= contexts.size()) {
		throw std::logic_error("Cannot multiply_switch_inplace() by more contexts than available.");
	}
